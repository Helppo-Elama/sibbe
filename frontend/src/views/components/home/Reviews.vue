<template>
	<div data-aos="fade-up">
		<h3 class="museo museo-heading pb-5">Asiakkai&shy;den arvoste&shy;lut</h3>
		<v-col cols="12">
		...
		<div v-for="(star, i) in stars" :key="i" class="stars pb-4">
			<a :href="star.href"
				><img :src="star.image" :alt="star.site" :style="star.imageMaxWidth" class="stars pb-3"
			/></a>
			<div :style="starsStyles(star.initLevel)" class="rating text-center"></div>
			<div class="rating-shadow text-center">★★★★★</div>
		</div>
		<img :src="logo.image" :alt="logo.alt" :style="logo.imageMaxWidth" class="stars pb-16" />
	</div>
</template>
...
.rating::before {
	content: "★★★★★";
	letter-spacing: 3px;
	background: linear-gradient(90deg, $star-background var(--percent), $star-color var(--percent));
