<template>
	<div>
		...
		>
			<v-list>
				<v-list-item v-for="item in appBar" :key="item.title" :to="item.path" class="color-yellow">
					<v-list-item-icon>
						<v-icon class="color-light">{{ item.icon }}</v-icon>
					</v-list-item-icon>
					<v-list-item-content class="color-light">{{ item.title }}</v-list-item-content>
				</v-list-item>
			</v-list>
		</v-navigation-drawer>
		...
		<v-app-bar app class="menu-icon-right">
			...
			<v-toolbar-items class="hidden-md-and-down d-xl-none light-on-dark">
				<v-btn text v-for="item in appBar" :key="item.title" :to="item.path" class="color-light">
					{{ item.title }}
				</v-btn>
			</v-toolbar-items>
		</v-app-bar>
